...
export SUB_CIDR="150.204.0.0/16"
...
export CUST_SUBNET_A=$(aws ec2 create-subnet --cidr-block 150.204.96.0/19  --vpc-id ${VPC_ID} --availability-zone ${AZ1} | jq -r .Subnet.SubnetId)
export CUST_SUBNET_B=$(aws ec2 create-subnet --cidr-block 150.204.128.0/19 --vpc-id ${VPC_ID} --availability-zone ${AZ2} | jq -r .Subnet.SubnetId)
export CUST_SUBNET_C=$(aws ec2 create-subnet --cidr-block 150.204.160.0/19 --vpc-id ${VPC_ID} --availability-zone ${AZ3} | jq -r .Subnet.SubnetId)
