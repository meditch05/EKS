...
3. Bastion 서버 점검 / TimeZone 변경
	...
	# date
        ...
        # ssh-keygen
			...
			=====================================
			# echo {\"insecure-registries\": [\"644960261046.dkr.ecr.ap-northeast-1.amazonaws.com\"]} | jq . > /etc/docker/daemon.json
...
5. EKS CLUSTER 생성 ( eksctl 사용. 0.20.0 부터는 managednodegreoup 생성시에 Error가 나니 Cluster / NodeGroup을 나눠서 만든다. )
	...
	# aws ssm get-parameter --name /aws/service/eks/optimized-ami/1.16/amazon-linux-2/recommended/image_id --region ap-northeast-1 --query "Parameter.Value" --output text
	ami-0b18567e6d3b05548   # 2020-06-09

	# cd EKS/cluster
	
	[ Cluster 생성 ]
	# date; eksctl create cluster --config-file=00.ap-northeast-1.eks-ds05599.create-cluster.yaml ; date
	
	[ NodeGreoup 생성 ]
	# date; eksctl create nodegroup --config-file=03.ap-northeast-1.eks-ds05599-1devops-2worker.managed.yaml ; date
	
	# aws eks describe-cluster --name ds05599 | jq '.cluster |.name, .endpoint, .resourcesVpcConfig'
...
8. Node별 POD 갯수 확인
   ...
   => Dual CIDR을   써도 aws-node-#### kube-proxy-### 은 Main CIDR로 생성된다.
	...
	ip-10-5-115-27.ap-northeast-1.compute.internal    Ready    worker   110m   v1.16.8-eks-e16311   t3a.small
	ip-10-5-120-232.ap-northeast-1.compute.internal   Ready    devops   110m   v1.16.8-eks-e16311   t3a.medium
	ip-10-5-188-123.ap-northeast-1.compute.internal   Ready    worker   110m   v1.16.8-eks-e16311   t3a.small	

	# kc get pod --all-namespaces -o wide | grep Running | awk '{print $8}' | sort | uniq -c
      5 ip-10-5-115-27.ap-northeast-1.compute.internal
      3 ip-10-5-120-232.ap-northeast-1.compute.internal
      5 ip-10-5-188-123.ap-northeast-1.compute.internal

	# kc get pod --all-namespaces -o wide | grep ContainerCreating | awk '{print $8}' | sort | uniq -c
      1 ip-10-5-115-27.ap-northeast-1.compute.internal
      1 ip-10-5-188-123.ap-northeast-1.compute.internal
	  
	# kc get pod --all-namespaces -o wide | awk '{print $8}' | grep -v NODE | sort | uniq -c
      6 ip-10-5-115-27.ap-northeast-1.compute.internal
      3 ip-10-5-120-232.ap-northeast-1.compute.internal
      6 ip-10-5-188-123.ap-northeast-1.compute.internal
	  
	# kc describe pod/busybox-ecr1-75648f4945-49sdq -n infra | tail -1   # ( 로그 확인해보면 CNI에서 IP 할당 실패로 ContainerCreating 상태임 )
	Warning  FailedCreatePodSandBox  4m47s (x271 over 9m38s)  kubelet, ip-10-5-115-27.ap-northeast-1.compute.internal  (combined from similar events): Failed create pod sandbox: rpc error: code = Unknown desc = failed to set up sandbox container "7acdc447cb5f77399ecf21089f2d46501fe9bdf0aa9288a71800c3a4a500a13b" network for pod "busybox-ecr1-75648f4945-49sdq": networkPlugin cni failed to set up pod "busybox-ecr1-75648f4945-49sdq_infra" network: add cmd: failed to assign an IP address to container
	...
	# kc get node -L ec2-type
	NAME                                               STATUS   ROLES    AGE     VERSION              EC2-TYPE
	ip-10-50-120-135.ap-northeast-1.compute.internal   Ready    worker   7m9s    v1.16.8-eks-e16311   t3a.small
	ip-10-50-142-29.ap-northeast-1.compute.internal    Ready    devops   6m56s   v1.16.8-eks-e16311   t3a.medium
	ip-10-50-158-107.ap-northeast-1.compute.internal   Ready    worker   7m15s   v1.16.8-eks-e16311   t3a.small
	
	# kc get pod --all-namespaces -o wide | awk '{print $8}' | grep -v NODE | sort | uniq -c
      8 ip-10-50-120-135.ap-northeast-1.compute.internal
     17 ip-10-50-142-29.ap-northeast-1.compute.internal
      8 ip-10-50-158-107.ap-northeast-1.compute.internal
	...
	# kc get pod -o wide --all-namespaces | grep "ip-10-50-142-29.ap-northeast-1.compute.internal" | grep Running | wc -l
	17
	...
	# kc get node -L ec2-type
	NAME                                              STATUS   ROLES    AGE     VERSION              EC2-TYPE
	ip-10-50-123-94.ap-northeast-1.compute.internal   Ready    devops   6m2s    v1.16.8-eks-e16311   t3a.medium
	ip-10-50-132-2.ap-northeast-1.compute.internal    Ready    worker   5m48s   v1.16.8-eks-e16311   t3a.small
	ip-10-50-97-145.ap-northeast-1.compute.internal   Ready    worker   6m16s   v1.16.8-eks-e16311   t3a.small
	
	# kc get pod --all-namespaces -o wide | awk '{print $8}' | grep -v NODE | sort | uniq -c
     17 ip-10-50-123-94.ap-northeast-1.compute.internal
      8 ip-10-50-132-2.ap-northeast-1.compute.internal
      8 ip-10-50-97-145.ap-northeast-1.compute.internal
	# kc get pod --all-namespaces -o wide | grep Running | awk '{print $8}'  | sort | uniq -c
     12 ip-10-50-123-94.ap-northeast-1.compute.internal
      2 ip-10-50-132-2.ap-northeast-1.compute.internal     # ( Network interface가 eth0 1개만 붙어있고, Secondary Private IPs가 비어있음. ENIConfig.yaml 에서 Subnet을 잘못 지정했음 )
      5 ip-10-50-97-145.ap-northeast-1.compute.internal
	  
	# kc get pod --all-namespaces -o wide | grep Running | awk '{print $8}'  | sort | uniq -c
     12 ip-10-50-123-94.ap-northeast-1.compute.internal
      5 ip-10-50-132-2.ap-northeast-1.compute.internal
      5 ip-10-50-97-145.ap-northeast-1.compute.internal
	
	# kc get pod --all-namespaces -o wide | grep -v Running | awk '{print $8}' | sort | uniq -c
	
###########################################################################################################
# Default Namespace를 infra로 변경 ( 손아프니까 )
############################################################################################################
...
>     annotations: {service.beta.kubernetes.io/aws-load-balancer-type: nlb}
	...
	# cd nginx-ingress
	# helm install nginx-ingress --namespace infra -f values.yaml.edit stable/nginx-ingress  --version v1.39.1
...
2. 생성 확인
	...
	service/nginx-ingress-controller        LoadBalancer   172.20.203.54   ab29712e0d4ba4bcc916fb6f4c935379-a3da055390627c00.elb.ap-northeast-1.amazonaws.com   80:30692/TCP,443:30567/TCP   21m
...
3. Worker Node에만 POD들이 생성되도록 Chart Update
	...
	busybox-deployment-ecr1-75bdbb5ffd-5bh5b         1/1     Running   0          4m40s   10.5.0.75     ip-10-5-188-123.ap-northeast-1.compute.internal   <none>           <none>
	busybox-deployment-ecr1-75bdbb5ffd-6hl2k         1/1     Running   0          4m42s   10.5.66.222   ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
	nginx-ingress-controller-pvh97                   1/1     Running   0          20m     10.5.6.184    ip-10-5-188-123.ap-northeast-1.compute.internal   <none>           <none>
	nginx-ingress-controller-zg85b                   1/1     Running   0          20m     10.5.71.32    ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
	nginx-ingress-default-backend-6d7985b7ff-5kdvc   1/1     Running   0          60s     10.5.88.111   ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
...
2. EKS가 사용하는 VPC_ID 확인
	# export VPC_ID=$(eksctl utils describe-stacks --region=ap-northeast-1 --cluster=${EKS_CLUSTER_NAME} | grep OutputValue | grep vpc | cut -d"\"" -f2)	
...
4. AZ별 Sub-CIDR 용 Subnet 생성
	...
	# export AZ2=ap-northeast-1b
	# export AZ3=ap-northeast-1c
...
6. Sub-CIDR 용 Subnet 을 VPC의 AZ별 라우팅 테이블에 연결
	...
	eksctl-ds05599-cluster/PrivateRouteTableAPNORTHEAST2A => rtb-0916ebb8934a4ab9e
	eksctl-ds05599-cluster/PrivateRouteTableAPNORTHEAST2B => rtb-03fcb781f4f5713d9
	eksctl-ds05599-cluster/PrivateRouteTableAPNORTHEAST2C => rtb-033102996bad6734d
...
13. nginx-ingress 호출 테스트 ( 안될껄 )
	# curl http://ab29712e0d4ba4bcc916fb6f4c935379-a3da055390627c00.elb.ap-northeast-1.amazonaws.com/asdlkjfskajdf
14. EKS의 보안그룹 ( ControlPlaneSecurityGroup ) 수정  => Sub-CIDR 에 생성되는 POD들과 EKS ControlPlane 대역(172.x.x.x)과 통신이 되지 않는 현상 조치
	...
	# eksctl-ds05599-nodegroup-devops-new/SG					sg-0443e5afda7c0c622	...
	# eks-cluster-sg-ds05599-647076920						sg-0592750418c54c4a3	...
	# eksctl-ds05599-nodegroup-worker-new/SG					sg-07982106e46d7d52b	...
	# eksctl-ds05599-cluster/ClusterSharedNodeSecurityGroup	sg-08040113e12d6d1bb	...
	# eksctl-ds05599-cluster/ControlPlaneSecurityGroup		sg-0dd182c2553e9d341	...
	
	[ managed NodeGroup 일 경우 ]
	
	※ 2개 managed node group 을 생성하면 아래 처럼 총 3개의 Security Group 이 보임 ( 3개 모두에 Inbound rule 추가 )
	eks-cluster-sg-ds05599-647076920						sg-0592750418c54c4a3	...
	eksctl-ds05599-cluster/ClusterSharedNodeSecurityGroup	sg-08040113e12d6d1bb	...   # 여기서 빼면 CNI 사용을 못해서 POD가 "ContainerCreating" 상태에서 멈춰버림
	eksctl-ds05599-cluster/ControlPlaneSecurityGroup		sg-0dd182c2553e9d341	...
...
1. 기존 nodegroup 확인 ( AMI가 kubenetes 1.15 용이므로 1.16 용으로 변경하자 )
	# eksctl get  nodegroup --cluster ds05599
	CLUSTER         NODEGROUP       CREATED                 MIN SIZE        MAX SIZE        DESIRED CAPACITY        INSTANCE TYPE   IMAGE ID
	ds05599       devops          2020-06-09T00:59:23Z    1               2               1                       t3a.medium      ami-08a18de5609e8f781
	ds05599       worker          2020-06-09T00:59:23Z    2               3               2                       t3a.small       ami-08a18de5609e8f781
2. 새로운 nodeGroup 설정파일 확인
	# diff 01.ap-northeast-1.eks-ds05599-1devops-2worker.yaml  02.ap-northeast-1.eks-ds05599-1devops-2worker.ami.chg.yaml
	15c15
	<   - name: devops
	---
	>   - name: devops-new
...
3. 새로운 nodegroup 용으로 clusterconfig 파일 변경 / nodegroup 생성
	# eksctl create nodegroup --config-file=02.ap-northeast-1.eks-ds05599-1devops-2worker.unmanaged.ami.chg.yaml
 
	[ 새로운 nodegroup, POD 생성 확인 ] => 새로운 Node에 생성된 POD는 Dual CIDR을 사용하게됨
	# eksctl get nodegroup --cluster ds05599
	CLUSTER         NODEGROUP       CREATED                 MIN SIZE        MAX SIZE        DESIRED CAPACITY        INSTANCE TYPE   IMAGE ID
	ds05599       devops          2020-06-09T00:59:23Z    1               2               1                       t3a.medium      ami-08a18de5609e8f781
	ds05599       devops-new      2020-06-09T02:02:29Z    1               2               1                       t3a.medium      ami-0b18567e6d3b05548
	ds05599       worker          2020-06-09T00:59:23Z    2               3               2                       t3a.small       ami-08a18de5609e8f781
	ds05599       worker-new      2020-06-09T02:02:29Z    2               3               2                       t3a.small       ami-0b18567e6d3b05548
	
	#  kc get node; kc get pod -n infra -o wide
	NAME                                              STATUS   ROLES        AGE     VERSION
	ip-10-5-126-161.ap-northeast-1.compute.internal   Ready    worker       2m4s    v1.16.8-eks-e16311
	ip-10-5-173-4.ap-northeast-1.compute.internal     Ready    worker       65m     v1.15.10-eks-bac369
	ip-10-5-182-180.ap-northeast-1.compute.internal   Ready    management   2m38s   v1.16.8-eks-e16311
	ip-10-5-189-242.ap-northeast-1.compute.internal   Ready    management   66m     v1.15.10-eks-bac369
	ip-10-5-191-218.ap-northeast-1.compute.internal   Ready    worker       2m1s    v1.16.8-eks-e16311
	ip-10-5-99-154.ap-northeast-1.compute.internal    Ready    worker       65m     v1.15.10-eks-bac369
	NAME                                             READY   STATUS    RESTARTS   AGE    IP              NODE                                              NOMINATED NODE   READINESS GATES
	nginx-ingress-controller-5569c                   1/1     Running   0          22m    10.5.110.204    ip-10-5-99-154.ap-northeast-1.compute.internal    <none>           <none>
	nginx-ingress-controller-782dq                   1/1     Running   0          65s    100.64.88.122   ip-10-5-126-161.ap-northeast-1.compute.internal   <none>           <none>
	nginx-ingress-controller-9jqn7                   1/1     Running   0          102s   100.64.0.255    ip-10-5-191-218.ap-northeast-1.compute.internal   <none>           <none>
	nginx-ingress-controller-nv2l2                   1/1     Running   0          33m    10.5.180.240    ip-10-5-173-4.ap-northeast-1.compute.internal     <none>           <none>
	nginx-ingress-default-backend-5b967cf596-wzhc9   1/1     Running   0          16m    10.5.167.155    ip-10-5-189-242.ap-northeast-1.compute.internal   <none>           <none>
...
5. 기존 nodegroup 삭제
	# eksctl delete nodegroup --cluster ds05599 --name=workers
	# eksctl delete nodegroup --cluster ds05599 --name=devops
...
############################################################################################################
1. NodeGroup 생성
	# eksctl create nodegroup --config-file=03.ap-northeast-1.eks-ds05599-1devops-2worker.managed.yaml
	# eksctl get nodegroup --cluster ds05599
	CLUSTER         NODEGROUP               CREATED                 MIN SIZE        MAX SIZE        DESIRED CAPACITY        INSTANCE TYPE   IMAGE ID
	ds05599       devops-new-managed      2020-06-09T04:14:41Z    1               2               1                       t3a.medium
	ds05599       worker-new-managed      2020-06-09T04:14:41Z    2               3               2                       t3a.small
...
3. 기존 nodegroup 삭제
	# eksctl delete nodegroup --cluster ds05599 --name=workers-new
	# eksctl delete nodegroup --cluster ds05599 --name=devops-new
		=> eks에서 cordon 시키고, drain 시킨다는 메시지가 뜨고
		=> Node가 Ready,SchedulingDisabled => NotReady,SchedulingDisabled => 삭제됨
...
2. 구성 확인
	...
	service/nginx-ingress-controller        LoadBalancer   172.20.203.54   ab29712e0d4ba4bcc916fb6f4c935379-a3da055390627c00.elb.ap-northeast-1.amazonaws.com   80:30692/TCP,443:30567/TCP   3h16m   app.kubernetes.io/component=controller,app=nginx-ingress,release=nginx-ingress
	...
	NAME                                                 READY   STATUS    RESTARTS   AGE   IP              NODE                                              NOMINATED NODE   READINESS GATES
	pod/busybox-deployment-ecr1-75bdbb5ffd-5bh5b         1/1     Running   0          20m   100.64.0.75     ip-10-5-188-123.ap-northeast-1.compute.internal   <none>           <none>
	pod/busybox-deployment-ecr1-75bdbb5ffd-6hl2k         1/1     Running   0          20m   100.64.66.222   ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
	pod/nginx-ingress-controller-pvh97                   1/1     Running   0          36m   100.64.6.184    ip-10-5-188-123.ap-northeast-1.compute.internal   <none>           <none>
	pod/nginx-ingress-controller-zg85b                   1/1     Running   0          35m   100.64.71.32    ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
	pod/nginx-ingress-default-backend-6d7985b7ff-5kdvc   1/1     Running   0          16m   100.64.88.111   ip-10-5-115-27.ap-northeast-1.compute.internal    <none>           <none>
################################################
...
3. Ingress 호출 테스트 ( NLB에 할당된 Public IP가 어떨땐 2개고, 어떨땐 3개여 ㅡ_ㅡ;;; )
	# nslookup ab29712e0d4ba4bcc916fb6f4c935379-a3da055390627c00.elb.ap-northeast-1.amazonaws.com
...
5. Ingress 호출 테스트
	# nslookup a0f2bb0e1275c4c5aab93c7ec8e9f87c-218b841f9b10b36c.elb.ap-northeast-1.amazonaws.com | grep Address
...
################################################
3. Ingress 호출 테스트 ( NLB에 할당된 Public IP가 어떨땐 2개고, 어떨땐 3개여 ㅡ_ㅡ;;; )
	# nslookup a6ac6cd5746ab4510ab3d2a5a2ced4f2-601267f4a9605765.elb.ap-northeast-1.amazonaws.com | grep Address
	Address:        10.16.0.2#53
